        ...
        if let Some(dot_token) = parser.take() {
            ...
            if let Some(lit) = parser.take() {
                let lit_int = match lit {
                    ...
                    _ => return Err(parser.emit_error("invalid field name")),
                };
                let LitInt { span, parsed, ty_opt } = lit_int;
                if let Some((_, _span)) = ty_opt {
                    return Err(parser.emit_error("type suffixes do not apply to field indexes"));
                }
                let field = parsed;
                let field_span = span;
            ...
            return Err(parser.emit_error("expected a field name"));
        }
        return Ok(expr);
    }
}
